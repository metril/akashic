...
// GetSecurityDescriptorBytes returns the raw, self-relative NT security
// descriptor bytes for the named file or directory (MS-SMB2 §2.2.37,
// InfoType=SMB2_0_INFO_SECURITY, AdditionalInformation=OWNER|GROUP|DACL).
//
// This method was added as a vendor patch in akashic-acl-capture to support
// NT ACL capture; see internal/vendor/go-smb2/PATCHES.md for details.
// It can be removed once upstream hirochachacha/go-smb2 merges equivalent
// functionality (track upstream PR #65 at github.com/hirochachacha/go-smb2/pull/65).
func (fs *Share) GetSecurityDescriptorBytes(name string) ([]byte, error) {
	name = normPath(name)
	...
	sd, err := f.getSecurityDescriptorBytes()
	if e := f.close(); err == nil {
		err = e
	}
	if err != nil {
...
// getSecurityDescriptorBytes returns the raw NT security descriptor bytes for
// the open file via SMB2 QUERY_INFO with InfoType=SMB2_0_INFO_SECURITY.
// Vendor patch; see GetSecurityDescriptorBytes on *Share for full context.
func (f *File) getSecurityDescriptorBytes() ([]byte, error) {
	req := &QueryInfoRequest{
